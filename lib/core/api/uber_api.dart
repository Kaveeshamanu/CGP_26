...
class UberApiException implements Exception {
  ...
  @override
  String toString() =>
      'UberApiException: $message (Code: $code, Status: $statusCode)';
}
    ...
    try {
      final response = await Dio().post(
        ...
        data: {
          ...
          'client_secret':
              'YOUR_CLIENT_SECRET', // In a real app, this would be securely stored
  ...
  Future<List<Ride>> getAvailableRides({
    ...
    try {
      ...
      for (final price in priceData) {
        ...
        // Create ride object according to the Ride model
        final ride = Ride(
          id: productId,
          userId: 'default-user', // Required field
          status: RideStatus.pending,
          transportType: _getTransportTypeFromName(price['display_name']),
          pickupAddress: 'Current Location',
          dropoffAddress: 'Destination',
          pickupLatitude: startLatitude,
          pickupLongitude: startLongitude,
          ...
          estimatedDuration: time['estimate'] != null
              ? (time['estimate'] ~/ 1000) // Convert milliseconds to seconds
              : 0,
          estimatedFare: price['high_estimate'] != null
              ? double.parse(price['high_estimate'].toString())
              : (price['minimum'] != null
                  ? double.parse(price['minimum'].toString())
                  : 0.0),
...
// Helper method to convert ride name to TransportType
  TransportType _getTransportTypeFromName(String? rideName) {
    if (rideName == null) return TransportType.car;
    ...
    } else if (name.contains('black') || name.contains('premium')) {
      return TransportType.luxury;
    } else {
